...
SCP_API scp_error_t scp_set_cipher_enabled(scp_cipher_t cipher, bool enabled);
...
SCP_API scp_error_t scp_connect(const scp_connect_config_t* config,
...
SCP_API scp_error_t scp_connect2(
...
SCP_API scp_error_t scp_disconnect(scp_session_t session);

// Check whether the session is currently connected and authenticated.
SCP_API bool scp_is_connected(scp_session_t session);

// Get the remote host string.
SCP_API const char* scp_session_get_host(scp_session_t session);

// Send a keepalive request. Useful before a long idle period.
SCP_API scp_error_t scp_keepalive(scp_session_t session);

// Get last SSH error message from libssh2.
SCP_API const char* scp_session_get_last_error(scp_session_t session);
...
SCP_API scp_error_t scp_list_dir(scp_session_t session,
                         const char* path,
                         scp_file_info_t** out_files,
                         uint32_t* out_count);

// Stat a remote path (file or directory).
SCP_API scp_error_t scp_stat(scp_session_t session,
                     const char* path,
                     scp_file_info_t* out_info);

// Create a directory on the remote host.
SCP_API scp_error_t scp_mkdir(scp_session_t session,
                      const char* path,
                      uint32_t permissions);

// Remove an empty directory on the remote host.
SCP_API scp_error_t scp_rmdir(scp_session_t session, const char* path);

// Delete a file on the remote host.
SCP_API scp_error_t scp_unlink(scp_session_t session, const char* path);

// Rename / move a remote file or directory.
SCP_API scp_error_t scp_rename(scp_session_t session,
                       const char* old_path,
                       const char* new_path);

// Change permissions of a remote file or directory.
SCP_API scp_error_t scp_chmod(scp_session_t session,
                      const char* path,
                      uint32_t permissions);

// Create an empty file on the remote host.
SCP_API scp_error_t scp_touch(scp_session_t session, const char* path);

// Get filesystem information (total / free bytes) for the given remote path.
SCP_API scp_error_t scp_df(scp_session_t session,
                   const char* path,
                   uint64_t* out_total,
                   uint64_t* out_free);

// Free a file list returned by `scp_list_dir`.
SCP_API void scp_free_file_list(scp_file_info_t* files, uint32_t count);
...
SCP_API scp_error_t scp_transfer_start(scp_session_t session,
...
SCP_API scp_error_t scp_batch_transfer(scp_session_t session,
                               const scp_batch_config_t* config);

// Pause a running transfer. Data written so far is preserved for resume.
SCP_API scp_error_t scp_transfer_pause(scp_transfer_t transfer);

// Resume a paused transfer.
SCP_API scp_error_t scp_transfer_resume(scp_transfer_t transfer);

// Cancel a transfer. Partial data may be removed (use pause for resumable stop).
SCP_API scp_error_t scp_transfer_cancel(scp_transfer_t transfer);

// Block until a transfer completes or fails.
SCP_API scp_error_t scp_transfer_wait(scp_transfer_t transfer);

// Poll the current progress of a transfer.
// Both `transferred` and `total` are in bytes.
SCP_API scp_error_t scp_transfer_get_progress(scp_transfer_t transfer,
                                      uint64_t* transferred,
                                      uint64_t* total);

// Release transfer handle resources.
SCP_API void scp_transfer_free(scp_transfer_t transfer);
